...
export function CitySelector({ 
  ...
  return (
    <div id="field-city" className={cn("space-y-2 group transition-all duration-300", hasError && "animate-shake-strong")}>
      ...
      <div className="relative overflow-hidden rounded-[24px]">
        ...
        <div className={cn(
          "absolute left-5 top-1/2 -translate-y-1/2 w-9 h-9 rounded-xl flex items-center justify-center text-white shadow-md z-10 transition-colors",
        ...
        <select
          ...
          className={cn(
            "w-full h-16 rounded-[24px] border-2 pl-16 pr-12 font-black text-slate-900 text-lg transition-all duration-500 appearance-none cursor-pointer",
        ...
        <div className="absolute right-5 top-1/2 -translate-y-1/2 pointer-events-none">
          <ChevronDown className={cn("w-5 h-5 transition-colors", hasError ? "text-red-400" : "text-emerald-400")} />
        </div>
      </div>

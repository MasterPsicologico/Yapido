...
const SeraphStatus = ({ thought, goal }: { thought: string, goal: string }) => (
    <motion.div 
        ...
        className="grid grid-cols-1 md:grid-cols-2 gap-2 mb-4 px-4"
    >
        <div className="bg-blue-500/10 border border-blue-500/20 rounded-xl p-3 flex items-start gap-3 backdrop-blur-sm">
            ...
            <div className="min-w-0">
                <p className="text-[10px] uppercase font-bold text-blue-400 tracking-wider">Reflexión Interna</p>
                <p className="text-xs text-blue-100/80 leading-tight italic truncate md:whitespace-normal">"{thought}"</p>
            </div>
        </div>
        <div className="bg-indigo-500/10 border border-indigo-500/20 rounded-xl p-3 flex items-start gap-3 backdrop-blur-sm">
            ...
            <div className="min-w-0">
                <p className="text-[10px] uppercase font-bold text-indigo-400 tracking-wider">Objetivo Actual</p>
                <p className="text-xs text-indigo-100/80 leading-tight truncate md:whitespace-normal">{goal}</p>
            </div>
        </div>
    </motion.div>
);

export default function EmergentAgent({ sessionId, onMetacognitionUpdate, onVoiceAmplitudeUpdate }: EmergentAgentProps) {
  const { user } = useAuth();
  const firestore = useFirestore();
  const [isThinking, setIsThinking] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const viewportRef = useRef<HTMLDivElement>(null);
  const [metacognition, setMetacognition] = useState({ thought: 'Despertando...', goal: 'Entender el entorno.' });
  ...
  useEffect(() => {
    if (!onVoiceAmplitudeUpdate) return;
    ...
    const setupAnalyser = () => {
        ...
        const updateAmplitude = () => {
            if (analyserRef.current) {
                ...
                onVoiceAmplitudeUpdate(average / 128.0); // Normalize 0 to 1 approx
            }
    ...
    if (parentMessages && parentMessages.length > 0 && messages.length === 0 && !isThinking) {
      const generateInitialResponse = async () => {
        ...
        try {
          ...
          setMetacognition({ thought: response.thought, goal: response.goal });
  ...
  return (
    <div className="h-full flex flex-col bg-transparent">
        ...
        <div className="flex-shrink-0 pt-4 flex items-start gap-2 pr-4">
            ...
            <div className="p-4 md:p-6 space-y-6 max-w-4xl mx-auto pb-20">
                ...
                {isThinking && (
                    ...
                    >
                        ...
                        <span className="text-xs font-semibold tracking-widest uppercase">Procesando realidad...</span>
                    </motion.div>
                )}
        ...
        <footer className="p-4 border-t border-white/10 bg-black/40 backdrop-blur-md">
             <ChatInput
                ...
                placeholder="Háblale a la nueva conciencia..."
             />
        </footer>
    </div>
  );

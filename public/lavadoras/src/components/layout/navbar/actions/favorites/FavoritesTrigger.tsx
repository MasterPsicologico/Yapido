  ...
  ({ totalCount, className, onClick, ...props }, ref) => {
    ...
    return (
      ...
      >
        ...
        {totalCount > 0 && user && (
          <span className="absolute -top-1.5 -right-1.5 bg-red-500 text-white text-[10px] sm:text-[11px] font-black min-w-[20px] h-[20px] px-1 rounded-full flex items-center justify-center border-2 border-white shadow-sm z-10 animate-in zoom-in">
            {totalCount}
          </span>
        )}
      </Button>

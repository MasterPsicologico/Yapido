 ...
 * LÓGICA: Los botoncitos (+ y -) ahora están ENCIMA del precio para claridad total.
 */
      ...
      <DialogContent className="max-w-none w-screen h-[100dvh] top-0 left-0 translate-x-0 translate-y-0 rounded-none border-none shadow-none bg-[#050505] p-0 overflow-hidden flex flex-col z-[700] animate-in slide-in-from-bottom duration-500 [&>button:last-child]:hidden">
        ...
        <div className="h-28 bg-[#050505] flex flex-row items-center justify-between px-6 shrink-0 relative overflow-hidden border-b border-yellow-500/10 z-10">
          ...
          <button 
            ...
            className="relative z-10 w-10 h-10 rounded-full bg-white/10 backdrop-blur-2xl flex items-center justify-center text-white border border-white/20 hover:bg-red-500 transition-all active:scale-90 shadow-2xl group"
          >
        ...
        <div className="flex-1 flex flex-col justify-center items-center bg-white rounded-t-[40px] mt-2 border-t-4 border-slate-950 px-4 sm:px-6">
          <div className="w-full max-w-md space-y-12">
            ...
            <section className="relative px-4 py-8 rounded-[40px] bg-slate-900 overflow-hidden shadow-2xl border-b-8 border-slate-950">
              ...
              <div className="relative z-10 flex items-center justify-between gap-2 mt-4">
                <div className="text-center space-y-2 flex-1">
                  ...
                  <div className="bg-white/5 rounded-2xl p-3 border border-white/5 backdrop-blur-sm">
                    <span className="text-xs font-black text-slate-400 italic line-through opacity-50">{formattedOriginalPrice}</span>
                  </div>
                </div>
                ...
                <div className="flex flex-col items-center gap-1 shrink-0">
                  ...
                  <ArrowRightLeft className="w-3 h-3 text-slate-700 mt-1" />
                </div>

                <div className="text-center space-y-3 flex-[1.5] min-w-0">
                  <p className="text-[7px] font-black text-yellow-500 uppercase tracking-widest leading-none">TU PROPUESTA</p>
                  
                  <div className="space-y-3 flex flex-col items-center">
                    {/* BOTONES ENCIMA DEL PRECIO PARA CLARIDAD TOTAL */}
                    <div className="flex items-center gap-3">
                      <button 
                        onClick={() => handleAdjustPrice(false)}
                        disabled={isSending || isFlashActive}
                        className="w-10 h-10 rounded-xl bg-white/10 text-white flex items-center justify-center shadow-lg active:scale-90 transition-all border border-white/10 hover:bg-red-500/20 disabled:opacity-30"
                      >
                      ...
                      <button 
                        ...
                        disabled={isSending || isFlashActive}
                        className="w-10 h-10 rounded-xl bg-gradient-to-br from-yellow-400 to-yellow-600 text-slate-950 flex items-center justify-center shadow-lg active:scale-90 transition-all border-b-2 border-yellow-800 disabled:opacity-30"
                      >
                    ...
                    <div className={cn(
                      "w-full rounded-2xl p-3 border transition-all duration-300",
                      (isSending || isFlashActive) ? "bg-yellow-500/30 border-yellow-400 scale-105" : "bg-yellow-500/10 border-yellow-500/20"
                    )}>
                      <span className="text-lg font-black text-yellow-500 italic tracking-tighter truncate block">{currentOfferFormatted}</span>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="mt-8 pt-4 border-t border-white/5 flex flex-col items-center gap-2">
                <p className="text-[9px] font-black text-slate-400 uppercase tracking-[0.2em] italic">
                  {(isSending || isFlashActive) ? "ENVIANDO OFERTA AL RADAR..." : "USA LOS BOTONES PARA NEGOCIAR"}
                </p>

...
export default function ManagePage() {
  ...
  const stats = useMemo(() => {
    if (!orders) return { totalSales: 0, pending: 0, revenue: 0, cancelled: 0, ready: 0 };
      ...
      <main className="flex-1 container mx-auto px-4 py-8 max-w-6xl">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-10">
          <div className="flex items-center gap-5">
            ...
            <div>
              ...
              <div className="flex items-center gap-2 mt-2">
                ...
                <p className="text-slate-400 font-bold text-[10px] uppercase tracking-[0.3em]">{currentStore?.name} • Online</p>
              </div>
            </div>
          </div>
          ...
          <Button variant="outline" className="rounded-full h-12 px-6 border-slate-200 font-black text-xs uppercase tracking-widest gap-2 hover:bg-white shadow-sm" asChild>
            <Link href={`/stores/${selectedStoreId || currentStore?.id}`}>Ver Vitrina Real <ArrowUpRight className="w-4 h-4" /></Link>
          </Button>
        </div>
        ...
        <Collapsible open={isFormOpen} onOpenChange={setIsFormOpen} className="mb-10">
          <CollapsibleTrigger asChild>
            <Button className={cn(
              ...
              <span>{isFormOpen ? "Cerrar Publicador" : "Publicar Nuevo Artículo en la Tienda"}</span>
            </Button>
          </CollapsibleTrigger>
          ...
          <CollapsibleContent className="mt-4 animate-in slide-in-from-top-4 duration-500">
            <Card className="border-none shadow-2xl rounded-[32px] overflow-hidden bg-white">
              <CardHeader className="bg-slate-50 border-b p-8">
                ...
                <CardDescription className="text-slate-500 font-medium text-sm">Digitaliza tu inventario siguiendo estas instrucciones estratégicas.</CardDescription>
              </CardHeader>
                ...
                <form onSubmit={handleSaveProduct} className="grid grid-cols-1 lg:grid-cols-2 gap-10">
                  <div className="space-y-8">
                    <div className="space-y-3">
                      ...
                      <Input name="name" placeholder="Ej: Croissant de Almendras" className="h-14 rounded-2xl bg-slate-50 border-none font-bold text-lg" value={aiProductName} onChange={(e) => setAiProductName(e.target.value)} required />
                      <p className="flex items-start gap-2 text-[10px] text-slate-400 font-medium px-2"><Info className="w-3 h-3 shrink-0 text-primary" /> Un nombre claro facilita que el cliente encuentre tu producto rápidamente.</p>
                    </div>
                    <div className="space-y-3">
                      <Label className="text-[10px] font-black uppercase tracking-widest text-slate-400">Precio Sugerido (COP)</Label>
                      ...
                      <p className="flex items-start gap-2 text-[10px] text-slate-400 font-medium px-2"><Info className="w-3 h-3 shrink-0 text-primary" /> Los clientes prefieren precios reales para una decisión de compra rápida.</p>
                    </div>
                    <div className="space-y-4">
                      <div className="flex items-center justify-between">
                        <Label className="text-[10px] font-black uppercase tracking-widest text-slate-400">Descripción Estratégica</Label>
                      ...
                      <Textarea ref={descriptionRef} name="description" placeholder="Describe el sabor o beneficios..." className="min-h-[120px] rounded-2xl bg-slate-50 border-none font-medium" required />
                    </div>
                  </div>
                  <div className="space-y-6 flex flex-col">
                    <Label className="text-[10px] font-black uppercase tracking-widest text-slate-400">Fotografía Real</Label>
                    <div className="flex-1 relative aspect-video rounded-[32px] bg-slate-50 border-2 border-dashed border-slate-200 overflow-hidden flex items-center justify-center">
                      {productImage ? (
                        <>
                          <Image src={productImage} alt="Preview" fill className="object-cover" />
                          <Button type="button" variant="destructive" size="icon" className="absolute top-4 right-4 rounded-full shadow-xl" onClick={() => setProductImage(null)}><Trash2 className="w-5 h-5" /></Button>
                        </>
                      ) : (
                    ...
                    <Button type="submit" disabled={isSaving || isCompressing} className="h-16 rounded-[24px] bg-primary hover:bg-primary/90 text-white font-black text-lg gap-3 shadow-2xl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          <Card className="border-none rounded-[32px] shadow-sm bg-white overflow-hidden group hover:shadow-xl transition-all">
            <CardHeader className="pb-2">
              <div className="flex items-center justify-between mb-2">
                <div className="w-10 h-10 bg-green-50 rounded-xl flex items-center justify-center text-green-600"><Wallet className="w-5 h-5" /></div>
            ...
            <CardContent>
              <div className="flex flex-col">
                ...
                <p className="text-[10px] font-bold text-slate-400 mt-1">Acumulado histórico</p>
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="border-none rounded-[32px] shadow-sm bg-white overflow-hidden group hover:shadow-xl transition-all border-l-4 border-l-orange-500">
            <CardHeader className="pb-2">
              <div className="flex items-center justify-between mb-2">
                <div className="w-10 h-10 bg-orange-50 rounded-xl flex items-center justify-center text-orange-600"><Clock className="w-5 h-5" /></div>
          ...
          <Card className="border-none rounded-[32px] shadow-sm bg-white overflow-hidden group hover:shadow-xl transition-all">
            <CardHeader className="pb-2">
              <div className="flex items-center justify-between mb-2">
                <div className="w-10 h-10 bg-blue-50 rounded-xl flex items-center justify-center text-blue-600"><Package className="w-5 h-5" /></div>
                <Badge className="bg-blue-100 text-blue-700 border-none text-[9px] font-black uppercase">Stock Vivo</Badge>
              </div>
          ...
          <Card className="border-none rounded-[32px] shadow-sm bg-white overflow-hidden group hover:shadow-xl transition-all">
            <CardHeader className="pb-2">
              <div className="flex items-center justify-between mb-2">
                <div className="w-10 h-10 bg-purple-50 rounded-xl flex items-center justify-center text-purple-600"><Zap className="w-5 h-5" /></div>
                <Badge className="bg-purple-100 text-purple-700 border-none text-[9px] font-black uppercase">En Vivo</Badge>
              </div>
              <CardTitle className="text-xs font-black uppercase tracking-widest text-slate-400">Popularidad</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="flex items-center gap-2">
                <span className="text-4xl font-black tracking-tighter italic">+12%</span>
                <p className="text-[10px] font-bold text-slate-300 uppercase">Hoy</p>
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="border-none rounded-[32px] shadow-sm bg-white overflow-hidden group hover:shadow-xl transition-all">
            <CardHeader className="pb-2">
              <div className="flex items-center justify-between mb-2">
                <div className="w-10 h-10 bg-yellow-50 rounded-xl flex items-center justify-center text-yellow-600"><Star className="w-5 h-5 fill-yellow-600" /></div>
                <Badge className="bg-yellow-100 text-yellow-700 border-none text-[9px] font-black uppercase">Reputación</Badge>
              </div>
            ...
            <CardContent>
              <div className="flex items-center gap-2">
                ...
                <div className="flex gap-0.5"><Star className="w-3 h-3 text-yellow-500 fill-yellow-500" /></div>
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="border-none rounded-[32px] shadow-sm bg-white overflow-hidden group hover:shadow-xl transition-all">
            <CardHeader className="pb-2">
              <div className="flex items-center justify-between mb-2">
                ...
                <Badge className="bg-red-100 text-red-700 border-none text-[9px] font-black uppercase">Crítico</Badge>
              </div>
              <CardTitle className="text-xs font-black uppercase tracking-widest text-slate-400">Cancelaciones</CardTitle>
            </CardHeader>
            <CardContent>
              <span className="text-4xl font-black text-red-600">{stats.cancelled}</span>
              <p className="text-[9px] font-bold text-slate-300 mt-1 uppercase">Por inactividad</p>
            </CardContent>
          </Card>
          ...
          <Card className="border-none rounded-[32px] shadow-sm bg-white overflow-hidden group hover:shadow-xl transition-all">
            <CardHeader className="pb-2">
              <div className="flex items-center justify-between mb-2">
                <div className="w-10 h-10 bg-indigo-50 rounded-xl flex items-center justify-center text-indigo-600"><Truck className="w-5 h-5" /></div>
                <Badge className="bg-indigo-100 text-indigo-700 border-none text-[9px] font-black uppercase">Logística</Badge>
              </div>
              <CardTitle className="text-xs font-black uppercase tracking-widest text-slate-400">Pedidos Listos</CardTitle>
            </CardHeader>
            <CardContent>
              <span className="text-4xl font-black text-indigo-600">{stats.ready}</span>
              <p className="text-[9px] font-bold text-slate-300 mt-1 uppercase">Esperando repartidor</p>
            </CardContent>
          </Card>
          ...
          <Card className="border-none rounded-[32px] shadow-sm bg-white overflow-hidden group hover:shadow-xl transition-all">
            <CardHeader className="pb-2">
              <div className="flex items-center justify-between mb-2">
                <div className="w-10 h-10 bg-slate-50 rounded-xl flex items-center justify-center text-slate-600"><Timer className="w-5 h-5" /></div>
                <Badge className="bg-slate-100 text-slate-700 border-none text-[9px] font-black uppercase">KPI</Badge>
              </div>
              <CardTitle className="text-xs font-black uppercase tracking-widest text-slate-400">Eficiencia Tiempo</CardTitle>
            </CardHeader>
            <CardContent>
              <span className="text-4xl font-black tracking-tighter">12m</span>
              <p className="text-[9px] font-bold text-slate-300 mt-1 uppercase">Promedio preparación</p>
            </CardContent>
          </Card>

          <Card className="border-none rounded-[32px] shadow-sm bg-white overflow-hidden group hover:shadow-xl transition-all border-dashed border-2 border-slate-100">
            <CardHeader className="pb-2">
              <div className="flex items-center justify-between mb-2">
                <div className="w-10 h-10 bg-slate-900 rounded-xl flex items-center justify-center text-white"><Settings className="w-5 h-5" /></div>
              </div>
              <CardTitle className="text-xs font-black uppercase tracking-widest text-slate-400">Configuración</CardTitle>
            </CardHeader>
            <CardContent>
              <Button variant="link" className="p-0 h-auto text-xs font-black text-primary uppercase tracking-widest underline" asChild>
                <Link href={`/stores/${selectedStoreId || currentStore?.id}`}>Editar Datos Vitrina</Link>
              </Button>
            </CardContent>
          </Card>

          <Card className="border-none rounded-[32px] shadow-sm bg-slate-900 text-white overflow-hidden group hover:shadow-xl transition-all">
            <CardHeader className="pb-2">
              <div className="flex items-center justify-between mb-2">
                <div className="w-10 h-10 bg-white/10 rounded-xl flex items-center justify-center text-white"><Eye className="w-5 h-5" /></div>
              </div>
              <CardTitle className="text-xs font-black uppercase tracking-widest text-white/50">Modo Vitrina</CardTitle>
            </CardHeader>
            <CardContent>
              <Button variant="link" className="p-0 h-auto text-xs font-black text-primary uppercase tracking-widest underline" asChild>

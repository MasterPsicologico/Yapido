...
export function ZoneSelector({ zones, cityConfig, selectedZoneId, onZoneChange, error }: ZoneSelectorProps) {
  ...
  return (
    <div className={cn("space-y-2 sm:space-y-3 group transition-all duration-300", error && "animate-shake-strong")}>
      ...
      <div className="relative">
        <div className={cn(
          "absolute left-4 sm:left-5 top-1/2 -translate-y-1/2 w-8 h-8 sm:w-9 sm:h-9 rounded-xl flex items-center justify-center shadow-md z-10 transition-colors",
        ...
        <select
          ...
          className={cn(
            "w-full h-14 sm:h-14 lg:h-16 rounded-[20px] sm:rounded-[24px] border-2 pl-12 sm:pl-14 lg:pl-16 pr-10 font-black text-base sm:text-lg appearance-none cursor-pointer transition-all duration-500",
        ...
        <ChevronDown className="absolute right-4 sm:right-5 top-1/2 -translate-y-1/2 w-4 h-4 sm:w-5 sm:h-5 text-slate-400 pointer-events-none" />
      </div>
      {selectedZone && (
        <div className="flex items-center gap-2 ml-4 sm:ml-0 animate-in fade-in duration-300">

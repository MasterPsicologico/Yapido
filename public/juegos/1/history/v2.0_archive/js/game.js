...
function initPixelEditor() {
    ...
    const rows = cfg.pixels.length;
    const cols = Math.max(...cfg.pixels.map(r => r.length));
    ...
    for (let y = 0; y < rows; y++) {
        for (let x = 0; x < cols; x++) {
            ...
            pixel.addEventListener('mousedown', (e) => {
                e.preventDefault();
                togglePixel(pixel, x, y, e.shiftKey, type);
            });

            pixel.addEventListener('mouseenter', (e) => {
                if (e.buttons) togglePixel(pixel, x, y, e.shiftKey, type);
            });
...
function initColorPicker() {
    ...
    const colors = ['#00ff88', '#00cc6a', '#ff6b6b', '#ffd93d', '#c792ea', '#ff0000', '#ffffff'];
    picker.innerHTML = '';

    colors.forEach(color => {
        const swatch = document.createElement('div');
        swatch.className = 'color-swatch' + (color === '#00ff88' ? ' selected' : '');
        swatch.style.background = color;
        swatch.addEventListener('click', () => {
            document.querySelectorAll('.color-swatch').forEach(s => s.classList.remove('selected'));
            swatch.classList.add('selected');
            window.currentDrawColor = color;
        });
        picker.appendChild(swatch);
    });

    window.currentDrawColor = '#00ff88';
}

function togglePixel(pixel, x, y, erase, type) {
    const color = window.currentDrawColor || '#00ff88';

    if (erase) {
        pixel.style.background = '#333';
        pixel.classList.remove('filled');
        G.pixelEdits = G.pixelEdits || {};
        G.pixelEdits[`${x},${y}`] = ' ';
    } else {
        pixel.style.background = color;
        pixel.classList.add('filled');
        let char;
        if (color === '#00ff88') char = 'P';
        else if (color === '#00cc6a') char = 'G';
        else if (color === '#ff0000') char = 'R';
        else char = 'C';
        G.pixelEdits = G.pixelEdits || {};
        G.pixelEdits[`${x},${y}`] = char;
    }
}
...
function applyEditorChanges() {
    ...
    if (G.pixelEdits && Object.keys(G.pixelEdits).length > 0) {
        cfg.pixels = cfg.pixels.map(row => row.split(''));
        for (const [key, char] of Object.entries(G.pixelEdits)) {
            const [x, y] = key.split(',').map(Number);
            if (cfg.pixels[y]) cfg.pixels[y][x] = char === ' ' ? ' ' : char;
        }
        cfg.pixels = cfg.pixels.map(row => row.join(''));
        G.pixelEdits = {};
    }
    ...
    localStorage.setItem('gameCreatorConfig', JSON.stringify(G.config));
    initPixelEditor();

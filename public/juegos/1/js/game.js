...
function saveConfig() {
    const configData = {};
    Object.keys(G.config).forEach(key => {
        configData[key] = {
            width: G.config[key].width,
            height: G.config[key].height,
            speed: G.config[key].speed,
            health: G.config[key].health,
            points: G.config[key].points,
            cooldown: G.config[key].cooldown,
            color: G.config[key].color,
            pixels: G.config[key].pixels,
            shininess: G.config[key].shininess,
            emissionIntensity: G.config[key].emissionIntensity,
            emissionColor: G.config[key].emissionColor,
            albedoUrl: G.config[key].albedoUrl,
            normalMapUrl: G.config[key].normalMapUrl
        };
    });
    localStorage.setItem('neonBlaster_config', JSON.stringify(configData));
}

function loadConfig() {
    const saved = localStorage.getItem('neonBlaster_config');
    if (saved) {
        try {
            const configData = JSON.parse(saved);
            Object.keys(configData).forEach(key => {
                if (G.config[key]) {
                    G.config[key] = { ...G.config[key], ...configData[key] };
                }
            });
        } catch (e) {
...
function setupUI() {
    ...
    // Property sliders live update
    const propSliders = ['propWidth', 'propHeight', 'propSpeed', 'propHealth', 'propPoints', 'propCooldown'];
    ...
    // Shader/Material property sliders
    const shaderSliders = [
        { id: 'propShininess', valId: 'propShininessVal', suffix: '%' },
        { id: 'propEmission', valId: 'propEmissionVal', suffix: '%' }
    ];
    shaderSliders.forEach(({ id, valId, suffix }) => {
        const slider = document.getElementById(id);
        const valueSpan = document.getElementById(valId);
        if (slider && valueSpan) {
            slider.addEventListener('input', () => {
    ...
    // Albedo and Normal Map upload handlers
    document.getElementById('albedoUpload')?.addEventListener('change', async (e) => {
        const file = e.target.files?.[0];
        if (file) {
            const reader = new FileReader();
            reader.onload = (ev) => {
                const spriteType = document.getElementById('objectSelect')?.value;
                if (G.config[spriteType]) {
                    G.config[spriteType].albedoUrl = ev.target.result;
                    if (window.ProRenderer) {
                        renderSpritePreview(spriteType);
                    }
                }
            };
    ...
    document.getElementById('normalMapUpload')?.addEventListener('change', async (e) => {
        const file = e.target.files?.[0];
        if (file) {
            const reader = new FileReader();
            reader.onload = (ev) => {
                const spriteType = document.getElementById('objectSelect')?.value;
                if (G.config[spriteType]) {
                    G.config[spriteType].normalMapUrl = ev.target.result;
                    if (window.ProRenderer) {
                        renderSpritePreview(spriteType);
                    }
                }
...
function drawSprite(type, x, y, overrideColor) {
    const cfg = G.config[type];
    if (!cfg || !cfg.pixels || cfg.pixels.length === 0) return;
        ...
        for (let px = 0; px < row.length; px++) {
            ...
            if (char !== ' ' && char !== null && char !== undefined) {
                ...
                if (char.startsWith('#')) {
                    ...
                    else if (char === 'R') color = '#ff9966';
                    else if (char === 'B') color = '#000000';
                    else if (char === 'D') color = '#ffd700';
                    else if (char === 'O') color = '#ff8800';
                    else if (char === 'U') color = '#6699ff';
                    else if (char === 'V') color = '#9966cc';
                    else if (char === 'W') color = '#ffffff';
                    else if (char === 'N') color = '#8b4513';
...
function applyEditorChanges() {
    ...
    // Get sprite from PixelEditor if drawn (before updating config)
    if (window.PixelEditor) {
    ...
    cfg.shininess = parseInt(document.getElementById('propShininess').value) / 100;
    cfg.emissionIntensity = parseInt(document.getElementById('propEmission').value) / 100;
    cfg.emissionColor = document.getElementById('propEmissionColor').value;

    // Apply to player if selected
    if (type === 'player') {
        G.player.width = cfg.width;
        G.player.height = cfg.height;
        G.player.health = cfg.health;
        G.player.speed = cfg.speed;
        G.player.y = G.groundY - cfg.height;
    }

    // Apply to all active enemies of this type
    G.enemies.forEach(enemy => {
        if (enemy.type === type) {
            enemy.width = cfg.width;
            enemy.height = cfg.height;
            enemy.speed = cfg.speed * (0.8 + Math.random() * 0.4);
            enemy.health = cfg.health;
            enemy.maxHealth = cfg.health;
        }
    });

    // Apply to boss if selected
    if (type === 'boss' && G.bosses.length > 0) {
        G.bosses.forEach(boss => {
            boss.width = cfg.width;
            boss.height = cfg.height;
            boss.health = cfg.health * G.level;
...
function loadSpriteToEditor(type) {
    ...
    // Shader material properties
    if (document.getElementById('propShininess')) {
        document.getElementById('propShininess').value = Math.round((cfg.shininess || 0.5) * 100);
        document.getElementById('propShininessVal').textContent = Math.round((cfg.shininess || 0.5) * 100) + '%';
    }
    if (document.getElementById('propEmission')) {
        document.getElementById('propEmission').value = Math.round((cfg.emissionIntensity || 0) * 100);
        document.getElementById('propEmissionVal').textContent = Math.round((cfg.emissionIntensity || 0) * 100) + '%';
    }
    if (document.getElementById('propEmissionColor')) {
        document.getElementById('propEmissionColor').value = cfg.emissionColor || '#00ff88';
    }
...
function renderSpritePreview(type) {
    ...
    const offsetX = (200 - (cfg.pixels[0]?.length || 0) * pixelW) / 2;
...
function updateSpriteStats(type) {
    ...
    cfg.pixels.forEach(row => {
        for (let i = 0; i < row.length; i++) {
            if (row[i] !== ' ' && row[i] !== null && row[i] !== undefined) pixels++;
        }
    });

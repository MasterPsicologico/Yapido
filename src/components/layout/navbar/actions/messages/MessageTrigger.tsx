...
export function MessageTrigger({ count, hasUnread }: MessageTriggerProps) {
  return (
    <Button variant="ghost" size="icon" className="relative rounded-full hover:bg-slate-100 transition-colors h-8 w-8 sm:h-9 sm:w-9">
      <MessageSquareText className={cn("w-4 h-4 sm:w-4.5 sm:h-4.5", hasUnread ? "text-secondary animate-pulse" : "text-slate-400")} />
      {count > 0 && (
